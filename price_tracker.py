"""
REI Price Tracker - Bright Data Web Scraper API
----------------------------------------------------
Track prices, availability, and product data from REI.

Docs  : https://docs.brightdata.co.kr/scraping-automation/web-scraper-api/overview
Info  : https://brightdata.co.kr/products/insights/price-tracker/rei
"""
...
# Search for "REI" to find the matching Web Scraper (format: gd_xxxxxxxxxx).
...
if not DATASET_ID:
    raise EnvironmentError(
        ...
        "2. Search for 'REI' and copy the Web Scraper ID.\n"
...
def track_prices(urls: list[str], **kwargs) -> list[dict]:
    """
    Collect price data for a list of REI product URLs.
...
def discover_by_keyword(keyword: str, limit: int = 50, **kwargs) -> list[dict]:
    """
    Discover REI products by keyword search.
...
def discover_by_category(category_url: str, limit: int = 100, **kwargs) -> list[dict]:
    """
    Discover all products from a REI category page.

    Example:
        results = discover_by_category("https://rei.com/s?k=headphones", limit=200)
    """
...
if __name__ == "__main__":
    ...
    sample_urls = [
        "https://www.rei.com/product/sample-item-123456",
        # Add more REI product URLs here
    ]
    results = track_prices(sample_urls)

    output_file = "rei_prices.json"
    ...
    # cat_results = discover_by_category("https://rei.com/category/sample", limit=50)
